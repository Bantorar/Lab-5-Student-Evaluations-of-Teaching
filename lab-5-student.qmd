...
format:
  html:
    embed-resources: true   # plots and figures embedded in the HTML file
    code-tools: true        # source code visible and collapsible
execute:
  echo: true                # all code and output will be shown
  warning: true
  message: true
---
...
teacher_eval <- read.csv("data-raw/teacher_evals.csv")
```
...
summary(teacher_eval)
```

The dataset contains student evaluations of teaching (SET) collected from a university in Poland during the 2020–2021 academic year, when instruction was fully online due to the COVID-19 pandemic. Each row records an evaluation response related to a specific question on a standardized 9-question survey assessing teaching effectiveness. In addition to SET scores, the dataset includes variables about the instructor (e.g., gender, degree, seniority), course-level statistics (e.g., response rate, percent of students who failed), and contextual factors like the number of participants. The rich combination of survey and performance data enables analysis of patterns and potential biases in student evaluations.
...
colnames(teacher_eval)
head(teacher_eval)
```

Each row in the dataset represents the response to a specific survey question for a given instructor-course combination. That is, the unit of observation is identified by the combination of teacher_id, course_id, and question_no. This structure allows the dataset to store one row per question answered for each course taught by an instructor.
...
write_csv(teacher_evals_clean, file = "data-clean/teacher_evals_clean.csv")
```
...
# Step 1: Get one row per instructor
instructors_unique <- teacher_evals_clean %>%
  select(teacher_id, academic_degree, seniority, sex) %>%
  distinct(teacher_id, .keep_all = TRUE)

# Step 2: Count each demographic variable
degree_counts <- instructors_unique %>%
  count(academic_degree, sort = TRUE)

seniority_counts <- instructors_unique %>%
  count(seniority, sort = TRUE)

sex_counts <- instructors_unique %>%
  count(sex, sort = TRUE)
...
degree_counts <- instructors_unique %>% count(academic_degree, sort = TRUE)
seniority_counts <- instructors_unique %>% count(seniority, sort = TRUE)
sex_counts <- instructors_unique %>% count(sex, sort = TRUE)
...
The majority of instructors in the dataset hold a doctorate degree, with fewer holding master's or professional degrees. Most instructors are tenured, followed by tenure-track and first-year faculty. The gender distribution is relatively balanced, though there is a slightly higher number of male instructors than female instructors.
...
questions_per_course <- teacher_evals_clean %>%
  group_by(teacher_id, course_id) %>%
  summarise(n_questions = n_distinct(question_no), .groups = "drop")

num_all_nine <- questions_per_course %>%
  filter(n_questions == 9) %>%
  nrow()

total_combinations <- nrow(questions_per_course)
prop_all_nine <- num_all_nine / total_combinations

tibble(all_nine = num_all_nine, total = total_combinations, proportion = prop_all_nine)
```
...
#| warning: false
# code chunk for Q9
q9_ratings <- teacher_evals_clean %>%
  filter(question_no == 901) %>%
  group_by(teacher_id) %>%
  summarise(avg_q1_score = mean(SET_score_avg, na.rm = TRUE)) %>%
  arrange(avg_q1_score)

q9_lowest <- slice_head(q9_ratings, n = 1)
q9_highest <- slice_tail(q9_ratings, n = 1)
...
#| warning: false
# code chunk for Q10
q10_fail_rate <- teacher_evals_clean %>%
  filter(seniority == 1) %>%
  group_by(teacher_id) %>%
  summarise(avg_fail_rate = mean(percent_failed_cur, na.rm = TRUE)) %>%
  arrange(avg_fail_rate)

q10_lowest <- slice_head(q10_fail_rate, n = 1)
q10_highest <- slice_tail(q10_fail_rate, n = 1)
...
#| warning: false
# code chunk for Q11
q11_response <- teacher_evals_clean %>%
  filter(sex == "female", academic_degree == c("dr" , "prof")) %>%
  group_by(teacher_id) %>%
  summarise(avg_resp_share = mean(resp_share, na.rm = TRUE)) %>%
  arrange(avg_resp_share)

q11_lowest <- slice_head(q11_response, n = 1)
q11_highest <- slice_tail(q11_response, n = 1)
