...
# Step 1: Count number of unique questions per teacher-course combo
questions_per_course <- teacher_evals_clean %>%
  group_by(teacher_id, course_id) %>%
  summarise(n_questions = n_distinct(question_no), .groups = "drop")

# Step 2: Count how many combinations used all 9 questions
num_all_nine <- questions_per_course %>%
  filter(n_questions == 9) %>%
  nrow()

# Step 3: Total combinations for reference
...
q11_response <- teacher_evals_clean %>%
  filter(
    sex == "female",
    academic_degree %in% c("doctorate", "professor")
  ) %>%
  group_by(teacher_id) %>%
...
#| warning: false
# code chunk for Q11
q12_response <- teacher_evals_clean %>%
  filter(
    sex == "female",
...
q12_lowest  <- slice_head(q12_response, n = 1)
q12_highest <- slice_tail(q12_response, n = 1)
